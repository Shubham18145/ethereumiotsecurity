...
def printmulti(clf, X, Y, name, figname):
    ...
    train_score = cross_val_score(estimator=clf, X=X, y=Y, cv=10)
    Y_predicted = cross_val_predict(estimator=clf, X=X, y=Y, cv=10)
...
printmulti(lrclf, X, Y, "Logistic Regression Classifier", "logisticregression")

nbclf = GaussianNB()
printmulti(nbclf, X, Y, "Gaussian Classifier", "gaussianclassifier")

rfclf = RandomForestClassifier(n_estimators=100)
printmulti(rfclf, X, Y, "Random Forest Classifier", "randomforest")

svmclf = svm.SVC(kernel='linear')  # Linear Kernel
printmulti(svmclf, X, Y, "Support Vector Machine Classifier", "svm")


knnClf = KNeighborsClassifier(n_neighbors=5)
printmulti(knnClf, X, Y, "K nearest neighbour Classifier", "knn")
